 ...
 if (isset($_GET['id'] ))
{
	...
	$result = mysql_query ('select user.username ,user.nim,user.nama,user.email,user.no_telepon,role.role_nama,status.status_nama from user,role,status where user.role_id=role.role_id and user.status_id=status.status_id and user.role_id=5 ');
	if (! $result)
	{
		echo "query gagal" ;
	}
			...
			<!-- start: Content -->
				...
				<div class="box span12">
					<div class="box-header" data-original-title>
						<h2><i class="halflings-icon user"></i><span class="break"></span>Peminjam</h2>
						<div class="box-icon">
				...
				<?php
                    while($row = mysql_fetch_assoc($result))
                    {
							...
							?>
							<a onclick="return confirm('Apakah anda yakin?')" href='peminjam.php?id=<?PHP echo $row['username']?>'><button type='submit' class='btn btn-danger icon-trash'> Delete</button></a></td>
							<?PHP
							
							echo "	<td style='text-align:center'>"; 
							echo "<a class='btn btn-success icon-ok' href='aktivasipeminjam.php?id=".$row['username']."'> Aktif</a>";
							echo "<a class='btn btn-warning icon-remove' href='nonaktivasipeminjam.php?id=".$row['username']."'> NonAktif</a></td>";
                            echo "</tr>";
                    }
                    //echo "</table>";	
