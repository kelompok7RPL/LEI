 ...
 if (isset($_GET['id'] ))
{
	...
	$result = mysql_query ('select user.username ,user.nim,user.nama,user.email,user.no_telepon,role.role_nama,status.status_nama from user,role,status where user.role_id=role.role_id and user.status_id=status.status_id and user.role_id=3 ');
	if (! $result)
	{
		echo "query gagal" ;
	}
	...
	<!-- start: Header -->
		...
		<div class="row-fluid">
			...
			<!-- start: Content -->
				...
				<!-- judul -->
				<center>
				<H1> Daftar Kepala Laboratorium </H1></center>
				
				<!-- akhir judul -->
				 <a href='tambahkalab.php' class="btn btn-success icon-plus"> Tambah Data Kalab</a><br><br>
				<!-- tabel -->
				<div class="row-fluid sortable">		
				<div class="box span12">
					<div class="box-header" data-original-title>
						<h2><i class="halflings-icon user"></i><span class="break"></span>Kalab</h2>
						<div class="box-icon">
				...
				<?php
                    while($row = mysql_fetch_assoc($result))
                    {
							...
							?>
							<a onclick="return confirm('Apakah anda yakin?')" href='kalab.php?id=<?PHP echo $row['username']?>'><button type='submit' class='btn btn-danger icon-trash'> Delete</button></a>
							<?PHP
							
							echo "<a class='btn btn-primary icon-edit'href='updatekalab.php?id=".$row['username']."'> Update</a></td>";
							echo "	<td style='text-align:center'>"; 
							echo "<a class='btn btn-success icon-ok' href='aktivasikalab.php?id=".$row['username']."'>Aktifkan</a>";
							echo "<a class='btn btn-warning icon-remove' href='nonaktivasikalab.php?id=".$row['username']."'>NonAktifkan</a></td>";
                            echo "</tr>";
                    }
                    //echo "</table>";	
