...
<?php
  include ('database.php');
   ...
   if (isset($_GET['id'] ))
   {
     ...
     $jadwal_id =$_GET['id'];
	 $sql = "select * FROM kelompok WHERE jadwal_id = '$jadwal_id' ";
	 $sql1=mysql_query($sql);
	 $row=mysql_fetch_assoc($sql1);
	 $jadwal_id=$row["jadwal_id"];
	 $jadwal_nama=$row['jadwal_nama'];
		...
		<div class="row-fluid">
			...
			<!-- start: Content -->
				...
				<H1> Update Jadwal</H1><br>
				 ...
				 <center>         
		            <form action='updatejadwal1.php'  method='POST' >
		               <table>
		                  <tr>
		                    <td> Jadwal</td>
		                    <td><input type="text" name="jadwal_nama" value='<?php echo $jadwal_nama?>' required readonly ></td>
		                  </tr>
		                  <tr>
		                    <td> Hari </td>
		                    <td><input type="text" name="jadwal_hari" value='<?php echo $jadwal_hari?>' required ></td>
		                  </tr>
		                  <tr>
		                    <td> jam</td>
		                    <td><input type="text" name="jadwal_jam"  value='<?php echo $jadwal_jam?>' required ></td>
		                  </tr>	
		                  <input type="hidden" name="jadwal_id"  value='<?php echo $jadwal_id?>' required >	                  
		            </table>  
		            <a href="jadwal2.php"><button type="submit" class="btn btn-warning icon-edit"> Update</button></a>  
		          </form>
