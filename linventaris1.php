	...
	<!-- start: Header -->
		...
		<div class="row-fluid">
			...
			<ul class="breadcrumb">
					...
					<a href="#">Laporan Inventaris</a></li>
			...
			<br>
      <form id="formku" action="linventaris1.php" method="POST">      
        <table align="center">
          <tr>
            <td> Jenis Peralatan</td>
            <td><select name="jenislaporan" >
            <option value="1"> Perkakas</option>
			<option value="3"> Komponen</option>
            <option value="2"> Alat Tulis Kantor</option>
            <option value="4"> Buku</option></select>
            </td>
          </tr>
        </table>
		    <a href='linventaris1.php'><button type="submit" class="btn btn-info icon-print"> Print</button></a>		  
        </center>
		
		<?php
		error_reporting(0);
		if (isset($_POST['jenislaporan']) )
                    { 

                            $id = $_POST['jenislaporan'];                            
					}
					if($id==1)
					{
						?>
						<script type="text/javascript">
						window.location = 'tampilanlperkakas.php?print=<?php echo "$id";?>';</script>
						<?php
					}
					if($id==2)
					{
						?>
						<script type="text/javascript">
						window.location = 'tampilanlATK.php?print=<?php echo "$id";?>';</script>
						<?php
					}
					if($id==3)
					{
						?>
						<script type="text/javascript">
						window.location = 'tampilanlkomponen.php?print=<?php echo "$id";?>';</script>
						<?php
					}
					if($id==4)
					{
						?>
						<script type="text/javascript">
						window.location = 'tampilanlbuku.php?print=<?php echo "$id";?>';</script>
						<?php
					}
