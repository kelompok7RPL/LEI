<?php
  include ('database.php');
   ...
   if (isset($_GET['id'] ))
   {
    ...
    $user ="SELECT jadwal_user.jadwal_user_id,user.nim,user.nama,user.no_telepon FROM kelompok join jadwal_user join user on kelompok.jadwal_id=jadwal_user.jadwal_id and jadwal_user.username=user.username where kelompok.jadwal_id='$kelompok_id'";
		...
		<div class="row-fluid">
			...
			<!-- start: Content -->
				...
				<ul class="breadcrumb">
					...
					<a href="adminkelompok.php">Kelompok</a>
					<i class="icon-angle-right"></i>
					</li>
					<li>
         ...
         <table border="3" class="table table-hover" align="center">
            <tr>
              <td style="text-align:center">Matakuliah : </td> 
          ...
          <table border="3" class="table table-hover" align="center">
           ...
           while ($row=mysql_fetch_assoc($user1)) {
              echo "<tr>";
              echo "<td style='text-align:center'>".$row["nim"]."</td>";
              echo "<td>".$row["nama"]."</td>";
              echo "<td style='text-align:center'>".$row["no_telepon"]."</td>";
              echo "<td style='text-align:center'>";
			  ?>
				<a onclick="return confirm('Apakah anda yakin?')" href='hapusanggota.php?id=<?PHP echo $row['jadwal_user_id']?>'><button type='submit' class='btn btn-danger icon-trash'> Hapus</button></a></td>
				<?PHP
